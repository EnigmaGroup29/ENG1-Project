...
public class MyGdxGame extends ApplicationAdapter {
	...
	public void render () {
        ...
        distanceFromPlayer = Math.sqrt(Math.pow(Infiltrator1.getComponent(1).getLocation().x - Auber.getComponent(1).getLocation().x, 2) + Math.pow(Infiltrator1.getComponent(1).getLocation().y - Auber.getComponent(1).getLocation().y, 2));
        System.out.println(distanceFromPlayer);
        if (distanceFromPlayer <= 100 && Infiltrator1.getComponent(0).isAbilityActive() == false && Infiltrator1.getComponent(0).isCooldownActive() == false  ) {
        	Infiltrator1.getComponent(0).toggleAbility();
        	System.out.println("hi");
        	timer.schedule(new InvisibilityDuration(), 5000);
        }
        
        if (!Infiltrator1.getComponent(0).isAbilityActive()) {
	...
	class InvisibilityCooldown extends TimerTask {
		
	    public void run() {
	    	Infiltrator1.getComponent(0).toggleCooldown();
	    }
	}
	
	class InvisibilityDuration extends TimerTask {
	    public void run() {
	    	System.out.println("Invisible");
	    	Infiltrator1.getComponent(0).toggleAbility();
	    	Infiltrator1.getComponent(0).toggleCooldown();
	    	timer.schedule(new InvisibilityCooldown(), 15000);
	    	
	    }
	}

	class ConfusionCooldown extends TimerTask {
	    public void run() {
	       System.out.println("Confused"); 
	    }
	}
	
	class ConfusionDuration extends TimerTask {
	    public void run() {
	       System.out.println("Confused"); 
	    }
	}
	...
	public abstract class Component{
        ...
        public abstract void toggleCooldown();
        public abstract boolean isCooldownActive();
	...
	public class Enemy extends Character{
        ...
        public void toggleCooldown() {
        	this.cooldownActive = !this.cooldownActive;
        }
	...
	public class Player extends Character{
        ...
        public void toggleAbility() {}
        public boolean isAbilityActive() { return false;}
        public void toggleCooldown() {}
